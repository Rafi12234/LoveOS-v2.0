...
/* ════════════════════════════════════════════════════════
   🌿 BRANCH VISUALIZATION — Animated git tree
   ════════════════════════════════════════════════════════ */
  ...
  return (
    ...
    >
      <div className="p-3 rounded-xl min-w-[140px]"
        ...
        {/* Activity sparkline */}
        <div className="mt-2 pt-2 flex items-end gap-[2px] h-6"
          ...
          {Array.from({ length: 14 }).map((_, i) => (
            <motion.div
              ...
              style={{
                background: `linear-gradient(to top, #3fb950, #a855f7)`,
                opacity: 0.4 + Math.random() * 0.6,
              }}
              animate={{
                height: [`${4 + Math.random() * 12}px`, `${6 + Math.random() * 16}px`, `${4 + Math.random() * 12}px`],
              }}
...
/* ════════════════════════════════════════════════════════
   📊 CONTRIBUTION GRID MODAL — Popup with full grid display
   ════════════════════════════════════════════════════════ */
function ContributionGridModal({ isOpen, onClose, commitCount }) {
  ...
  const colors = ['rgba(255,255,255,0.02)', '#0e4429', '#006d32', '#26a641', '#39d353'];
  const loveColors = ['rgba(255,255,255,0.02)', 'rgba(244,114,182,0.15)', 'rgba(244,114,182,0.3)', 'rgba(244,114,182,0.5)', 'rgba(244,114,182,0.8)'];
  const [useLove, setUseLove] = useState(false);
  const palette = useLove ? loveColors : colors;
          ...
          >
            ...
            >
              ...
              <div className="text-center mb-6">
                <h2 className="text-[16px] sm:text-[18px] md:text-[20px] terminal-text text-[#c9d1d9] tracking-widest font-bold mb-2">
                  ♥ LOVE CONTRIBUTIONS ♥
                </h2>
                <span className="text-[13px] sm:text-[14px] terminal-text text-[#3fb950]">
                  {commitCount} commits in the last year
                </span>
              </div>
              ...
              <div className="flex justify-center mb-6">
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  className="px-4 py-1.5 rounded text-[10px] sm:text-[11px] terminal-text transition-all font-semibold"
                  style={{
                    background: useLove ? 'rgba(244,114,182,0.15)' : 'rgba(63,185,80,0.15)',
                    border: `1.5px solid ${useLove ? 'rgba(244,114,182,0.5)' : 'rgba(63,185,80,0.5)'}`,
                    color: useLove ? '#f472b6' : '#3fb950',
                  }}
                >
                  {useLove ? '♥ Show Love Palette' : '● Show Code Palette'}
                </motion.button>
              </div>

              {/* Grid display */}
              <div className="flex gap-[6px] overflow-x-auto pb-6 justify-center">
                {Array.from({ length: weeks }).map((_, w) => (
                  <div key={w} className="flex flex-col gap-[6px] flex-shrink-0">
                    {Array.from({ length: days }).map((_, d) => {
                      const cell = cells[w * days + d];
                      return (
                        <motion.div
                          key={`${w}-${d}`}
                          className="w-[20px] h-[20px] rounded-[3px] cursor-pointer transition-all"
                          style={{ backgroundColor: palette[cell.level] }}
                          initial={{ scale: 0, opacity: 0 }}
                          animate={{ scale: 1, opacity: 1 }}
                          transition={{ delay: (w * days + d) * 0.008, duration: 0.3 }}
                          whileHover={{
                            scale: 1.4,
                            boxShadow: `0 0 16px ${useLove ? '#f472b6' : '#3fb950'}`,
                          }}
                        />
                      );
                    })}
              ...
              <div className="flex items-center gap-2 mt-8 justify-center">
                <span className="text-[10px] text-[#484f58] terminal-text mr-2">Less</span>
                {palette.map((c, i) => (
                  <div key={i} className="w-[14px] h-[14px] rounded-[2px]" style={{ backgroundColor: c }} />
                ))}
                <span className="text-[10px] text-[#484f58] terminal-text ml-2">More</span>
              </div>

              {/* Samanta text */}
              <div className="mt-8 text-center">
                <span className="text-[14px] terminal-text text-[#c9d1d9] tracking-wider font-semibold">
                  ~ Samanta's Love Journey ~
                </span>
              </div>
            </div>
          </motion.div>
...
/* ════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════ */
function ContributionGraph({ commitCount }) {
  ...
  return (
    <>
      <motion.div
        ...
        className="px-4 sm:px-5 md:px-6 py-6 sm:py-8 mb-8 flex flex-col items-center justify-center"
        style={{ borderTop: '1px solid rgba(48, 54, 61, 0.4)' }}
      >
        {/* Samanta title */}
        <motion.div
        ...
        >
          <span className="block text-[11px] sm:text-[12px] md:text-[13px] terminal-text text-[#8b949e] tracking-widest uppercase mb-2">
            LOVE CONTRIBUTIONS BY SAMANTA
          </span>
          <span className="block text-[10px] sm:text-[11px] terminal-text text-[#3fb950] font-semibold">
            {commitCount} commits in the last year
          </span>
        </motion.div>
        ...
        <motion.button
          ...
          whileHover={{ scale: 1.08, boxShadow: '0 0 20px rgba(244, 114, 182, 0.3)' }}
          whileTap={{ scale: 0.95 }}
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 1 }}
          className="px-6 sm:px-8 py-2.5 sm:py-3 rounded-lg text-[11px] sm:text-[12px] terminal-text font-semibold transition-all"
          style={{
        ...
        >
          <span className="flex items-center gap-2">
            <span>♥</span>
            <span>View Contribution Grid</span>
            <span>♥</span>
          </span>
        </motion.button>
        ...
        >
          <span className="text-[9px] sm:text-[10px] terminal-text text-[#8b949e] tracking-wide">
            Click to explore the journey of love in commits
          </span>
        </motion.div>
      </motion.div>

      {/* Contribution Grid Modal */}
...
/* ════════════════════════════════════════════════════════
   📊 DIFF STATS — File changes visualization
   ════════════════════════════════════════════════════════ */
      ...
      <div className="flex gap-[2px] ml-1">
        {Array.from({ length: blocks }).map((_, i) => (
          <motion.div
            ...
            style={{
              backgroundColor: i < addBlocks ? '#3fb950' : '#f85149',
            }}
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
...
/* ════════════════════════════════════════════════════════
   🔍 COMMIT DETAIL PANEL — Expanded commit view
   ════════════════════════════════════════════════════════ */
    ...
    >
      <div className="mt-3 rounded-xl overflow-hidden"
        ...
        <div className="px-4 py-2">
          <p className="text-[9px] terminal-text text-[#8b949e] mb-2 tracking-wider uppercase">
          ...
          {files.map((file, i) => (
            ...
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
                stroke={file.status === 'added' ? '#3fb950' : '#e6edf3'}
                strokeWidth="2" strokeLinecap="round">
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                <polyline points="14 2 14 8 20 8" />
              </svg>
              <span className="text-[10px] terminal-text text-[#e6edf3] flex-1 group-hover:text-love-cyan transition-colors" style={{ fontFamily: 'monospace' }}>
                {file.name}
              </span>
        ...
        >
          ...
          {[
            ...
            <motion.div
              ...
              style={{
                background: line.prefix === '+' ? 'rgba(63,185,80,0.08)' : 'transparent',
              }}
            >
              <span className="w-8 text-right text-[#484f58] pr-2 select-none flex-shrink-0">{line.num}</span>
              <span className={`w-3 text-center flex-shrink-0 ${line.prefix === '+' ? 'text-[#3fb950]' : 'text-transparent'}`}>
                {line.prefix}
              </span>
...
/* ════════════════════════════════════════════════════════
   🔎 SEARCH BAR — Animated commit search
   ════════════════════════════════════════════════════════ */
    ...
    >
      <div className="relative">
        <motion.div
          ...
          animate={{
            boxShadow: focused
              ? '0 0 0 2px rgba(168,85,247,0.4), 0 0 15px rgba(168,85,247,0.1)'
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg"
          ...
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
            stroke={focused ? '#a855f7' : '#484f58'} strokeWidth="2" strokeLinecap="round"
          ...
          <input
            ...
            className="flex-1 bg-transparent text-[12px] terminal-text text-[#e6edf3] 
          ...
          {value && (
            <motion.span
              ...
              animate={{ opacity: 1, scale: 1 }}
              className="text-[9px] terminal-text text-[#8b949e] flex-shrink-0"
            >
              {resultCount} found
            </motion.span>
          )}
...
/* ════════════════════════════════════════════════════════
   📈 STATS OVERVIEW — Summary statistics bar
   ════════════════════════════════════════════════════════ */
function StatsOverview({ commits }) {
  ...
  const stats = [
    { label: 'Commits', value: totalCommits, color: '#3fb950', icon: (
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
        <circle cx="12" cy="12" r="4" /><line x1="1.05" y1="12" x2="7" y2="12" /><line x1="17.01" y1="12" x2="22.96" y2="12" />
      </svg>
    )},
    { label: 'Tagged', value: tagged, color: '#a855f7', icon: (
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
        <path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z" /><line x1="7" y1="7" x2="7.01" y2="7" />
      </svg>
    )},
    { label: 'Authors', value: authors, color: '#f472b6', icon: (
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" />
      </svg>
    )},
    { label: 'Days', value: dates, color: '#00f0ff', icon: (
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
...
/* ════════════════════════════════════════════════════════
   🕐 COMMIT NODE — Single commit in the graph
   ════════════════════════════════════════════════════════ */
function CommitNode({ commit, index, total, isExpanded, onToggle, isLast }) {
  ...
  return (
    ...
    >
      {/* Date separator */}
      {(index === 0 || COMMIT_HISTORY.slice().reverse()[index - 1]?.date !== commit.date) && (
        <motion.div
      ...
      {/* Commit row */}
      <motion.div
        className="group px-5 py-3 flex items-start gap-3 cursor-pointer relative"
        style={{ borderBottom: '1px solid rgba(48, 54, 61, 0.2)' }}
        onClick={onToggle}
        animate={{
          backgroundColor: isHovered ? 'rgba(22, 27, 34, 0.4)' : 'transparent',
        }}
        transition={{ duration: 0.2 }}
      >
        {/* Hover accent line */}
        ...
        <div className="flex flex-col items-center pt-0.5 shrink-0 relative">
          ...
          {index > 0 && (
            <div className="w-[2px] h-3 absolute -top-3"
              style={{
                background: `linear-gradient(to bottom, rgba(48,54,61,0.3), ${nodeColor}40)`,
              }}
            />
          )}
          ...
          {!isLast && (
            <motion.div
              className="w-[2px] flex-1 mt-0.5 min-h-[20px]"
              style={{
                background: `linear-gradient(to bottom, ${nodeColor}40, rgba(48,54,61,0.3))`,
              }}
              initial={{ scaleY: 0 }}
              animate={{ scaleY: 1 }}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <motion.p
              className="text-sm text-[#e6edf3] font-medium truncate transition-colors duration-200"
              style={{ color: isHovered || isExpanded ? '#00f0ff' : '#e6edf3' }}
            >
              {commit.message}
            </motion.p>
          ...
          {/* Expanded detail */}
          <AnimatePresence>
            {isExpanded && <CommitDetail commit={commit} onClose={onToggle} />}
          </AnimatePresence>
        </div>
      </motion.div>
    </motion.div>
...
/* ════════════════════════════════════════════════════════
   📜 SCROLLBAR MARKERS — Commit density indicator
   ════════════════════════════════════════════════════════ */
      ...
      <div className="h-full rounded-full overflow-hidden"
        ...
        <motion.div
          ...
          style={{
            ...
            background: 'linear-gradient(to bottom, #3fb950, #a855f7, #f472b6)',
          }}
          transition={{ duration: 0.1 }}
        />
      </div>
      {/* Tagged commit markers */}
      {commits.map((c, i) => c.tag ? (
        <motion.div
          key={c.hash}
          className="absolute right-0 w-1.5 h-1.5 rounded-full"
          style={{
            top: `${(i / commits.length) * 100}%`,
            backgroundColor: c.tag === 'hotfix' ? '#f85149' : '#a855f7',
            boxShadow: `0 0 4px ${c.tag === 'hotfix' ? '#f85149' : '#a855f7'}`,
          }}
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          style={{ background: 'rgba(5, 5, 15, 0.95)', backdropFilter: 'blur(12px)' }}
          onClick={onClose}
        >
          ...
          <div className="absolute inset-0 pointer-events-none z-0">
            ...
            <motion.div className="absolute w-[280px] h-[280px] rounded-full"
              style={{
                bottom: '15%', right: '10%',
                background: 'radial-gradient(circle, rgba(63,185,80,0.08), transparent 70%)',
                filter: 'blur(60px)',
              }}
          ...
          >
            ...
            <motion.div className="absolute -inset-[1px] rounded-2xl pointer-events-none z-0"
              animate={{
                boxShadow: [
                  '0 0 20px rgba(244,114,182,0.15), 0 0 40px rgba(63,185,80,0.1)',
                  '0 0 30px rgba(244,114,182,0.25), 0 0 60px rgba(63,185,80,0.15)',
                  '0 0 20px rgba(244,114,182,0.15), 0 0 40px rgba(63,185,80,0.1)',
                ],
              }}
              transition={{ duration: 4, repeat: Infinity }}
            ...
            <div className="relative rounded-2xl overflow-hidden" style={{ background: 'rgba(13, 17, 23, 0.98)', border: '1px solid rgba(48, 54, 61, 0.7)' }}>
              {/* Header */}
              <div className="px-6 py-4 flex items-center justify-between relative z-10"
                ...
                <div>
                  ...
                  <p className="text-[12px] terminal-text text-[#8b949e] mt-1">Samanta's journey through commits</p>
                </div>
                <motion.button
                  onClick={onClose}
              ...
              <div className="px-6 py-8 max-h-[60vh] overflow-y-auto">
                <ContributionGraph commitCount={commitCount} isModalView={true} />
              </div>

              {/* Footer */}
              <div className="px-6 py-3 flex items-center justify-center"
                style={{ background: 'rgba(22, 27, 34, 0.4)', borderTop: '1px solid rgba(48, 54, 61, 0.3)' }}>
                <p className="text-[11px] terminal-text text-[#484f58]">
                  Each cell represents a day of love and code
                </p>
              </div>
            </div>
          </motion.div>
    ...
    >
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <motion.div className="absolute w-[400px] h-[400px] rounded-full"
          style={{
            top: '10%', left: '20%',
            background: 'radial-gradient(circle, rgba(63,185,80,0.06), transparent 70%)',
            filter: 'blur(60px)',
          }}
          animate={{ x: [0, 30, 0], y: [0, -20, 0] }}
          transition={{ duration: 10, repeat: Infinity }}
      ...
      >
        ...
        <motion.div className="absolute -inset-[1px] rounded-2xl pointer-events-none z-0"
          animate={{
            boxShadow: [
              '0 0 20px rgba(63,185,80,0.08), 0 0 40px rgba(168,85,247,0.05)',
              '0 0 30px rgba(63,185,80,0.12), 0 0 60px rgba(168,85,247,0.08)',
              '0 0 20px rgba(63,185,80,0.08), 0 0 40px rgba(168,85,247,0.05)',
            ],
          }}
          transition={{ duration: 4, repeat: Infinity }}
        ...
        <div className="relative rounded-2xl overflow-hidden"
          ...
          <div className="px-5 py-3.5 flex items-center justify-between relative z-10"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-[#e6edf3] flex items-center gap-2">
                  ...
                  <span className="text-[#484f58]">/</span>
                  <span className="text-love-cyan">commits</span>
                </h3>
                <p className="text-[9px] terminal-text text-[#484f58] flex items-center gap-1.5 mt-0.5">
                  ...
                  <span>•</span>
                  <span>{COMMIT_HISTORY.length} commits</span>
                  <span>•</span>
                  <span className="text-love-pink flex items-center gap-0.5">
                    <svg width="7" height="7" viewBox="0 0 24 24" fill="#f472b6">
                      <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                    </svg>
                    {HER_NAME}
                  </span>
            ...
            <div className="flex items-center gap-2">
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.05, background: 'rgba(63,185,80,0.15)' }}
                whileTap={{ scale: 0.95 }}
              ...
              >
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#3fb950" strokeWidth="2" strokeLinecap="round">
                  <rect x="3" y="3" width="18" height="18" rx="2" /><line x1="8" y1="3" x2="8" y2="21" /><line x1="16" y1="3" x2="16" y2="21" /><line x1="3" y1="8" x2="21" y2="8" /><line x1="3" y1="16" x2="21" y2="16" />
                </svg>
            ...
            {filteredCommits.length > 0 && (
              ...
              >
                <motion.div className="inline-flex items-center gap-2 px-4 py-2 rounded-full"
                  ...
                  <span className="text-[10px] terminal-text text-love-pink">
                    End of commit history — our story continues...
                  </span>
                </motion.div>
              </motion.div>
            )}
          ...
          {/* Contribution graph */}
          <ContributionGraph commitCount={COMMIT_HISTORY.length} />
          ...
          >
            <div className="flex items-center gap-3 text-[8px] terminal-text text-[#484f58]">
              <span className="flex items-center gap-1">
                <kbd className="px-1 py-0.5 rounded" style={{ background: 'rgba(48,54,61,0.4)', border: '1px solid rgba(48,54,61,0.5)' }}>ESC</kbd>
                close
              </span>
              <span>•</span>
              <span className="flex items-center gap-1">
                <kbd className="px-1 py-0.5 rounded" style={{ background: 'rgba(48,54,61,0.4)', border: '1px solid rgba(48,54,61,0.5)' }}>Click</kbd>
                expand
              </span>
            </div>
